...
```text
input = จำนวนเงินที่ของบิลที่ต้องนำไปคำนวนรับค่ามาเป็น int 
output = จำนวนเงินที่ลูกค้าจะต้องจ่ายซึ่งเป็นค่าบิลรวมกับค่า service และ VAT
constraints = จะต้องรับ input มาเป็น int เท่านั้น
```
...
```text
คิดแยกออกมาแต่ละตัวแปรว่า แต่ละตัวควรคิดอย่างไร และจะนำทุกตีวมาคิดออกมาเพื่อให้ได้ผลลัพธ์ออกมา โดยที่เมือ่ service < 50 จะได้เป็น 50 และเมื่อ service > 1000 ให้ service = 1000
```
...
```text
รับ input เข้ามาเป็นตัวเลขและนำไปคำนวนต่อโดยคำนวนหาค่า service ใช้สูตร * 0.1 และใช้  condition ในการกำหนด service ให้อยู่ในเกณฑ์ที่โจทย์กำหนดมา เพื่อที่จะนำไปคำนวนหาค่า  VAT ต่อไป สุดท้ายนำตัวแปรทุกตัวมาบวกกันเพื่อให้ได้คำตอบออกมา
```
...
```text
เพราะอยากทดลองว่าถ้า bill น้อยกว่า 50 จะได้เป็นปริมาณใดออกมา
```
...
```text
เนื่องจากอยากลองว่าหาก bill มากกว่า 1000 จะได้ค่าออกมาเป็นเท่าไหร่
```
...
```text
เนื่องจากอยู่ในเกณฑ์ 50 และ 1000
```
